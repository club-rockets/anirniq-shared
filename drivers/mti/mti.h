...
/** MTi configuration structure. Allows you to configure which data to get, and
 * their sampling rates. Setting a sampling rate to 0 will disable that data.
 */
typedef struct {
  uint16_t acceleration_sampling_rate;
  uint16_t attitude_sampling_rate;
  // uint16_t delta_v_sampling_rate;
  // uint16_t temperature_sampling_rate;
  // uint16_t rate_of_turn_sampling_rate;
  // uint16_t magnetic_field_sampling_rate;
} mti_config_t;
...
typedef struct {
  ...
  uint32_t last_data_time;              /** Timestamp of last timestamp*/
