...
void canSetFilter(canInstance_t *instance, canFilter_t *filter, enum canFilterMode mode, uint32_t filterId,
                  uint32_t fifoId)
{
    ...
    // fifo assignment
    if (fifoId)
        instance->instance->FFA1R |= (1 << filterId);
    else
        instance->instance->FFA1R &= ~(1 << filterId);
...
void can1Fifo0RegisterCallback(void (*callback)(void)) { can1Fifo0Callback = callback; }
...
void can1Fifo1RegisterCallback(void (*callback)(void)) { can1Fifo1Callback = callback; }
// get message from sf fifo return positive when new message available
...
void can1TXRegisterCallback(void (*callback)(void)) { can1TXCallback = callback; }

void can1WakeRegisterCallback(void (*callback)(void)) { can1WakeCallback = callback; }
...
uint32_t canSendPacket(canInstance_t *instance, uint32_t id, uint32_t remote, uint32_t dataL, void *data)
{
    ...
    if ((instance->instance->TSR & CAN_TSR_TME_Msk) == 0)
        return 0;
    // look for valid size
    if (dataL > 8)
        return 0;
...
void canPrintRxPacket(canRXpacket_t *packet)
{
    ...
    printf("id : %#03x \t rmt : %#02x \t fId : %#02x \t size : %#02x \t data:", packet->STID, packet->RTR, packet->FMI,
           packet->DLC);
...
void CAN1_RX0_IRQHandler(void)
{
    if (CAN1->RF0R & CAN_RF0R_FMP0_Msk) {
        ...
        memcpy(&(can1SfRxFifo0.rx[can1SfRxFifo0.end]), &(CAN1->sFIFOMailBox[0]),
               sizeof(can1SfRxFifo0.rx[can1SfRxFifo0.end]));
        // clear received frame from fifo
...
void CAN1_RX1_IRQHandler(void)
{
    if (CAN1->RF1R & CAN_RF0R_FMP0_Msk) {
        ...
        // read frame from fifo
        memcpy(&(can1SfRxFifo1.rx[can1SfRxFifo1.end]), &(CAN1->sFIFOMailBox[1]),
               sizeof(can1SfRxFifo1.rx[can1SfRxFifo1.end]));
        // clear received frame from fifo
        CAN1->RF0R |= CAN_RF0R_RFOM0;
    }
